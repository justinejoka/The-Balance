    ...
    <Container className="bg-gray-100 dark:bg-gray-900 py-20 px-10 flex flex-col gap-10">
      {latestPosts.map((post) => (
        <Link
          ...
          key={post?._id}
        >
          <div id="blog-content" className="flex flex-col md:flex-row gap-10 bg-white dark:bg-gray-800 rounded-md rounded-tr-md rounded-br-md hover:shadow-md duration-200">
                ...
                <div className="flex items-center gap-2">
                  {post?.categories &&
                    post.categories.map((item) => (
                      <p
                        key={item?._id}
                        className="text-xs uppercase text-blue-600 dark:text-blue-400 font-semibold"
                      >
                        {item?.title}
                      </p>
                    ))}
                </div>
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-white hover:text-orange-600 dark:hover:text-orange-400 duration-200 cursor-pointer">
                  {post?.title}
                </h2>
                <p className="text-gray-500 dark:text-gray-300">{post?.description}</p>
              </div>
